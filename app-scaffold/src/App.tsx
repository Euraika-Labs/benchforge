...
function Targets({ targets, adapters, packs, onRefresh, setMessage, openRunBuilder, openResultsForGroup, repairIntent, onRepairIntentConsumed }: { targets: Target[]; adapters: Adapter[]; packs: BenchmarkPack[]; onRefresh: () => Promise<void>; setMessage: (message: string) => void; openRunBuilder: (intent: RunBuilderIntent) => void; openResultsForGroup: (groupId: string, runId?: string) => void; repairIntent: TargetRepairIntent | null; onRepairIntentConsumed: () => void }) {
  ...
  return <section><div className="section-head"><h1>Targets</h1><div className="actions"><button disabled={validating === 'all'} onClick={() => validateAll().catch(error => setMessage(String(error)))}><RefreshCw size={16} />Validate all</button><label className="compact-select">Compare pack <select value={comparisonPackId} onChange={event => setComparisonPackId(event.target.value)}>{modelBenchmarkPacks.map(pack => <option key={pack.id} value={pack.id}>{pack.label}</option>)}</select></label><button disabled={!allComparisonTargetIds.length} title={allComparisonTargetIds.length ? `Compare enabled local targets with ${preferredCloudComparisonTargetIds.length} cloud target(s); priced cloud targets are preferred for capped cost estimates` : 'Add one enabled local model target and one enabled cloud model target'} onClick={openAllLocalCloudComparison}><ClipboardCheck size={16} />Compare local/cloud</button><button onClick={() => addMock().catch(error => setMessage(String(error)))}><Plus size={16} />Mock target</button><button onClick={() => addWorkerHarness().catch(error => setMessage(String(error)))}><Plus size={16} />Worker harness</button></div></div>
    ...
    <table><thead><tr><th>Name</th><th>Kind</th><th>Adapter</th><th>Stored</th><th>Health</th><th></th></tr></thead><tbody>{targets.map(t => {
...
function SettingsPage({ busy, targets, packs, setBusy, setMessage, refresh, openRunBuilder, openResultsForGroup }: { busy: boolean; targets: Target[]; packs: BenchmarkPack[]; setBusy: (busy: boolean) => void; setMessage: (message: string) => void; refresh: () => Promise<void>; openRunBuilder: (intent: RunBuilderIntent) => void; openResultsForGroup: (groupId: string, runId?: string) => void }) {
  ...
  async function downloadModel() {
    ...
    try {
      ...
      const job = await startHuggingFaceDownloadJob(repoId, plan.selectedFile, modelRevision || undefined, {
        ...
        autoCompareAfterStart: autoBenchmarkAfterStart && autoCompareAfterStart,
  ...
  async function startModelFromSelection(
    ...
    const job = await startHuggingFaceServerJob(selectedRepoId, selectedFilename || undefined, settings.port, settings.context, {
      ...
      autoCompareAfterStart: runBenchmarkAfterStart && compareAfterStart,
    });
    autoRegisterServerJobIdsRef.current.add(job.id);
    setServerJobs(current => mergeServerJob(current, job));
    setActiveServerJobId(job.id);
